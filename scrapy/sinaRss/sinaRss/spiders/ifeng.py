...
class ifengNewsSpider( CrawlSpider ):
    ...
    def __init__( self ):
        ...
        readin = codecs.open( 'sinaRss/spiders/ifeng_config.txt', "r", "utf-8" )
    ...
    def articleParse( self, response ):
        ...
        try:
			print ("getin");
			self.conn.commit()
			self.cur.execute('insert into allcontent( url, content ) values( %s, %s )', ( item['url'], content))
			self.conn.commit()
			print "getout"
        except:
